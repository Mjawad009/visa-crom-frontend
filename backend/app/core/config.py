...
class Settings(BaseSettings):
    ...
    def _force_asyncpg_driver(cls, v: str) -> str:
        """Railway's official Postgres template (and most managed
        Postgres providers) hand out a plain `postgresql://` URL, but
        this app talks to the DB async via asyncpg, which needs the
        `postgresql+asyncpg://` scheme explicitly — SQLAlchemy otherwise
        defaults to the sync psycopg2 driver, which isn't even
        installed, and the app fails to start. Normalizing here means
        it doesn't matter which format whatever's providing DATABASE_URL
        happens to use.
        """
