...
class CaseService:
    ...
    async def transition(
        ...
        if transition_key == "close_unsuccessful":
            ...
            # Reopening always lands on an active (non-terminal) stage —
            # see cases/seed.py — so it's always correct to clear both.
            case.is_closed = False
            case.closed_reason = None
            await self.db.commit()
            await self.db.refresh(case)
        else:
            # Any other transition might have advanced into the
            # *successful* terminal stage (post_visa_support), which —
            # unlike close_unsuccessful — has no dedicated transition key
            # of its own (it's reached via the same "advance" as every
            # other step). Check the resulting stage directly rather
            # than special-casing a transition key for it.
            refreshed = await self.workflow.get_instance_for_entity("case", str(case_id))
