...
class ReportsService:
    ...
    async def staff_case_workload(self) -> StaffWorkloadResponse:
        """Consultant user id -> open case count. Caller resolves names
        via the Users module if it wants to display them — Reports
        stays permission-agnostic about who's who, same discipline as
        every other function here."""
        counts = await get_case_counts_by_consultant(self.db)
        rows = [
            StaffWorkloadRow(user_id=user_id, open_case_count=count)
            for user_id, count in counts.items()
            if user_id != "unassigned"
        ]
        rows.sort(key=lambda r: r.open_case_count, reverse=True)
        return StaffWorkloadResponse(rows=rows)
