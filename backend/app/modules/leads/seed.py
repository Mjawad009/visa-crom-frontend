...
async def seed_lead_pipeline(db) -> None:
    ...
    transitions = [
        ...
        # Reopen a lead marked lost by mistake (or whose circumstances
        # changed) — drops it back to "Contacted" rather than "New",
        # since some contact history presumably already exists.
        TransitionDefinition(key="reopen", name="Reopen", from_stage_key="lost", to_stage_key="contacted"),
    ]

    existing = await service.get_definition_by_key("lead_pipeline")
    if existing:
        await service.sync_transitions("lead_pipeline", transitions)
        return
