...
class WorkflowEngineService:
    ...
    async def sync_transitions(self, definition_key: str, transitions: list) -> None:
        """Adds any transitions not already present on an existing
        definition, matched by (key, from_stage_key, to_stage_key).
        Lets a seed_*_pipeline() function evolve a pipeline already
        seeded in production (e.g. add a "reopen" transition later)
        without a destructive re-seed — every seed function calls this
        instead of just returning when the definition already exists,
        so redeploying picks up new transitions automatically."""
        definition = await self.get_definition_by_key(definition_key)
        if not definition:
            return

        stages_result = await self.db.execute(select(WorkflowStage).where(WorkflowStage.definition_id == definition.id))
        stage_by_key = {s.key: s for s in stages_result.scalars().all()}

        existing_result = await self.db.execute(
            select(WorkflowTransition).where(WorkflowTransition.definition_id == definition.id)
        )
        existing = {(t.key, t.from_stage_id, t.to_stage_id) for t in existing_result.scalars().all()}

        added = False
        for t in transitions:
            from_stage = stage_by_key.get(t.from_stage_key)
            to_stage = stage_by_key.get(t.to_stage_key)
            if not from_stage or not to_stage:
                continue
            signature = (t.key, from_stage.id, to_stage.id)
            if signature in existing:
                continue
            self.db.add(
                WorkflowTransition(
                    definition_id=definition.id, from_stage_id=from_stage.id, to_stage_id=to_stage.id,
                    key=t.key, name=t.name, required_permission=t.required_permission,
                )
            )
            added = True
